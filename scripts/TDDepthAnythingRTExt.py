...
venv_root = r"D:\TDDepthAnythingRT-main\.venv"
dll_dirs = [
    os.path.join(venv_root, r"Lib\site-packages\torch\lib"),
    os.path.join(venv_root, r"Lib\site-packages\torchaudio\lib"),
]
cuda_path = r"C:\Program Files\NVIDIA GPU Computing Toolkit\CUDA\v12.8\bin"
if os.path.exists(cuda_path):
    dll_dirs.append(cuda_path)
...
class TDDepthAnythingRTExt:
    ...
    def __init__(self, ownerComp: Any) -> None:
        ...
        self.tensor_pool = {}
        self.binding_buffer = [0, 0]
        self.fps_history = deque(maxlen=30)
        self.frame_count = 0
        self.total_time = 0.0

    def _initialize_tensors_once(self) -> None:
        """Lazy initialization of tensors (called once)."""
        if self._is_initialized or self.stream is None:
            return
        ...
        try:
            with self.stream:
                ...
                self.trt_input = torch.zeros((h, w), device=self.device)
                self.trt_output = torch.zeros((h, w), device=self.device)

                # Pre-allocate intermediate tensors
                self.tensor_pool["rgba"] = torch.zeros(
                    (4, h, w), device=self.device, dtype=torch.float32
                )

                self._is_initialized = True
                self.SafeLogger.info("Tensors initialized for optimized inference")
        except Exception as e:
            self.SafeLogger.warning(f"Tensor initialization failed: {e}")
    ...
    def _load_engine(self) -> Optional[trt.ICudaEngine]:
        ...
        try:
            ...
            with open(TRTbin, "rb") as f, trt.Runtime(
    ...
    def setupTensor(self) -> None:
        ...
        """
        with self.stream:
            self.trt_input = torch.zeros(
                (self.source.height, self.source.width), device=self.device
            )
            self.trt_output = torch.zeros(
                (self.source.height, self.source.width), device=self.device
            )
    ...
    def run(self) -> None:
        ...
        with self.EngineLock:
            if (
                self.ownerComp.par.Active.eval()
                and self.engine
                and hasattr(self.stream, "cuda_stream")
                and self.to_tensor
            ):
                ...
                if elapsed > 0:
                    ...
                    if len(self.fps_history) > 10:
                        ...
                        if self.frame_count % 30 == 0:
                            self.ownerComp.par.Modelstatus = f"{avg_fps:.1f} FPS"
    ...
    def _run_inference(self, img: torch.Tensor, output: torch.Tensor) -> None:
        ...
        """
        self.binding_buffer = [img.data_ptr()] + [output.data_ptr()]
        for i in range(self.engine.num_io_tensors):
            self.context.set_tensor_address(
                self.engine.get_tensor_name(i), self.binding_buffer[i]
            )
    ...
    def _process_output_tensor(self) -> None:
        ...
        with self.stream:
            if self.ownerComp.par.Normalize == "normal":
                ...
                self.trt_output = (self.trt_output - tensor_min) / (
                    tensor_max - tensor_min
                )

            rgba_tensor = torch.zeros(
                (4, self.trt_output.shape[0], self.trt_output.shape[1]),
                device=self.device,
                dtype=self.trt_output.dtype,
            )
            rgba_tensor[0, :, :] = self.trt_output

            output = TopCUDAInterface(
                self.source.width, self.source.height, 4, np.float32
            )
            self.scriptBuffer.copyCUDAMemory(
                rgba_tensor.permute(1, 2, 0).contiguous().data_ptr(),
                output.size,
                output.mem_shape,
                stream=self.stream.cuda_stream,
            )

    # --- OPTIMIZED ALTERNATIVE RUN METHOD (NEW) ---
    def run_optimized(self) -> None:
        """
        Optimized inference pipeline with pre-allocated buffers.
        Call this instead of run() for better performance.
        """
        with self.EngineLock:
            if not (self.ownerComp.par.Active.eval() and self.engine and self.stream):
                return
            ...
            with self.stream:
                self.to_tensor.update(self.stream.cuda_stream)
                self.trt_input = torch.as_tensor(self.to_tensor, device=self.device)
                normalized = self.normalize(self.trt_input[1:, :, :])
                self.trt_input = normalized.ravel()

            # Run inference
            self.binding_buffer = [
                self.trt_input.data_ptr(),
                self.trt_output.data_ptr(),
            ]
            for i in range(self.engine.num_io_tensors):
                self.context.set_tensor_address(
                    self.engine.get_tensor_name(i), self.binding_buffer[i]
            ...
            if elapsed > 0:
                self.fps_history.append(1.0 / elapsed)
                if len(self.fps_history) > 10 and self.frame_count % 30 == 0:
                    avg_fps = sum(self.fps_history) / len(self.fps_history)
                    self.ownerComp.par.Modelstatus = f"{avg_fps:.1f} FPS"
    ...
    def LoadModelThreaded(self) -> None:
        ...
        if self.Accelerate.model:
            self.ownerComp.par.Modelstatus = (
                f"{self.Accelerate.model_complete_name} already loaded."
            )
        ...
        if cached_dir:
            self.ownerComp.par.Modelstatus = (
                f"{self.Accelerate.model_complete_name} cached."
            )
            self.Logger.Info(
                f"Model already downloaded at {cached_dir}, loading from cache."
            )
    ...
    def LoadModelSuccess(self) -> None:
        ...
        self.ownerComp.par.Modelstatus = (
            f"{self.Accelerate.model_complete_name} loaded."
        )
    ...
    def AccelerateModelThreaded(self) -> None:
        ...
        if self.Accelerate.is_engine_built():
            self.ownerComp.par.Modelstatus = (
                f"{pathlib.Path(self.trt_path).name} already built."
            )
            self.Logger.Info(
                f"Existing TensorRT engine detected at {self.trt_path}; skipping acceleration."
            )
            return
        ...
        if self.Accelerate.model is None:
            self.Logger.Info(
                "Model not in memory; attempting to load (uses cache if present)."
            )
        ...
        if self.Accelerate.model:
            ...
            self.Logger.Error(
                'The model could not be loaded. Click "Download Model" first or check the cache.'
            )
    ...
    def AccelerateModelSuccess(self) -> None:
        ...
        if self.Accelerate.is_engine_built() and self.Accelerate.validate_engine():
            ...
            self.ownerComp.par.Modelstatus = (
                f"{pathlib.Path(self.trt_path).name} built."
            )
        else:
            self.Logger.Error(
                "Acceleration reported success but the engine is missing or invalid."
            )
        ...
        with self.EngineLock:
            ...
            if pathlib.Path(self.trt_path).exists():
                if not self.Accelerate.validate_engine():
                    self.SafeLogger.error(
                        f"TensorRT engine at {self.trt_path} failed validation; re-run acceleration."
                    )
                    return
                ...
                self.SafeLogger.error(
                    f"TensorRT engine file does not exist: {self.trt_path}"
                )
    ...
    def UnloadModelFromGPUThreaded(self) -> None:
        ...
        if self.ownerComp.par.Active.eval():
            ...
            self.Logger.Info(
                "Inference was turned off because the user requested to unload the model."
            )
